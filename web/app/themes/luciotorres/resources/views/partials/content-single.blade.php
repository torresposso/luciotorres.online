...
<article @php(post_class('h-entry w-full relative'))>
    ...
    <div class="max-w-3xl mx-auto px-4 md:px-0 pb-12">
        ...
        <div
            class="e-content font-serif text-lg md:text-2xl leading-relaxed prose prose-luciotorres max-w-none text-base-content/90 prose-headings:font-display prose-headings:font-black prose-headings:text-base-content prose-a:text-primary prose-a:font-bold hover:prose-a:text-secondary prose-a:transition-colors prose-strong:text-base-content">
            @php(the_content())
        </div>

        @if ($pag)
            <footer class="mt-8 pt-6 border-t border-base-300">
                <nav class="page-nav" aria-label="{{ __('Páginas', 'luciotorres') }}">
                    {!! $pag !!}
                </nav>
            </footer>
        @endif
