<div
        ...
        @if (has_nav_menu('primary_navigation'))
            <nav aria-label="{{ wp_get_nav_menu_name('primary_navigation') }}">
                {!! wp_nav_menu([
                    ...
                    'menu_class' =>
                        'flex gap-8 text-md font-black tracking-wide uppercase text-white/80 [&_a:hover]:text-secondary [&_a]:transition-colors',
                    'container' => false,
                    'echo' => false,
                ]) !!}
            </nav>
    ...
    <div class="navbar-end gap-3">
        ...
        <form role="search" method="get" action="{{ home_url('/') }}"
            class="hidden lg:flex items-center bg-white/5 border border-white/10 rounded-2xl px-3 py-1.5 text-white gap-2 w-40 focus-within:w-56 focus-within:border-secondary/50 transition-all duration-300">
        ...
        @if (has_nav_menu('primary_navigation'))
            <label for="main-drawer" tabindex="0" role="button"
                class="btn btn-ghost lg:hidden text-white p-2 ml-1 drawer-button" aria-label="Abrir navegación">
