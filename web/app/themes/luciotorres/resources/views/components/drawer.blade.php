...
<aside {{ $attributes->merge(['class' => 'drawer-side z-50']) }} aria-label="{{ __('Menú de navegación lateral', 'luciotorres') }}">
  <label for="main-drawer" aria-label="{{ __('Cerrar menú', 'luciotorres') }}" class="drawer-overlay"></label>
    ...
    <div class="flex flex-col flex-1 relative z-10 overflow-y-auto">
      ...
      <div class="px-6 py-2 bg-primary text-primary-content font-sans text-[8px] uppercase tracking-[0.25em] flex justify-between items-center select-none shrink-0 border-b border-primary/20">
        <span class="flex items-center gap-1.5 font-extrabold">
          ...
          {{ __('Caribe Colombiano', 'luciotorres') }}
        </span>
      ...
      <header class="navbar bg-base-100/90 backdrop-blur-md border-b border-base-300 px-6 min-h-18 flex items-center justify-between shrink-0">
        ...
        <div class="navbar-end">
          <label for="main-drawer" class="btn btn-ghost btn-circle hover:bg-primary/10 hover:text-primary hover:scale-105 active:scale-95 text-base-content/70 cursor-pointer flex items-center justify-center transition-all duration-300" aria-label="{{ __('Cerrar menú', 'luciotorres') }}" tabindex="0">
      ...
      <search class="px-6 py-5 border-b border-base-300/40 bg-base-100/30 block shrink-0">
        <form role="search" method="get" class="relative flex items-center w-full" action="{{ home_url('/') }}">
          <label class="sr-only" for="drawer-search">{{ _x('Buscar:', 'label', 'luciotorres') }}</label>
          <input
            id="drawer-search"
            type="search"
            ...
            placeholder="{{ esc_attr_x('Buscar artículos…', 'placeholder', 'luciotorres') }}"
      ...
      <nav class="py-6 drawer-nav-menu shrink-0" aria-label="{{ __('Navegación principal (móvil)', 'luciotorres') }}">
      ...
      @if(!empty($drawer_posts))
        @php
          ...
          $feat_cat_name = !empty($feat_cats) ? $feat_cats[0]->name : __('Destacado', 'luciotorres');
        ...
        <div class="px-6 py-6 bg-base-100/30 border-b border-base-300/40 shrink-0">
          <h3 class="font-sans text-[9px] uppercase tracking-[0.2em] text-primary font-extrabold mb-4 flex items-center gap-2">
            <span>{{ __('Historia Destacada', 'luciotorres') }}</span>
      ...
      <div class="px-6 py-6 border-b border-base-300/40 shrink-0 bg-primary/[0.01]">
        <div class="space-y-1 mb-4">
          <h3 class="font-sans text-[9px] uppercase tracking-[0.2em] text-primary font-extrabold">
            {{ __('Boletín Independiente', 'luciotorres') }}
          </h3>
          <p class="font-serif text-[10px] text-base-content/60 italic leading-relaxed">
            {{ __('Recibe nuestras crónicas y análisis técnicos directamente en tu correo.', 'luciotorres') }}
          </p>
        </div>
        ...
        <form class="flex gap-2">
          <input 
            ...
            name="email"
            id="drawer-email"
            placeholder="{{ esc_attr__('Tu correo electrónico…', 'luciotorres') }}" 
          ...
          <button 
            ...
            aria-label="{{ __('Suscribirse', 'luciotorres') }}">
            <span>{{ __('Unirse', 'luciotorres') }}</span>
