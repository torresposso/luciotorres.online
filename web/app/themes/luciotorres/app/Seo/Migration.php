...
/**
 * WP-CLI command that migrates Yoast SEO postmeta to _luciotorres_* keys.
 *
 * Usage: wp luciotorres migrate-seo
 *
 * Maps _yoast_wpseo_* postmeta to _luciotorres_* keys with
 * title variable expansion (%%title%%, %%sep%%, %%sitename%%, etc.).
 */
class Migration
{
    /**
     * Map of Yoast meta keys to luciotorres meta keys.
     *
     * @var array<string, string>
     */
    public const META_MAP = [
        '_yoast_wpseo_metadesc' => '_luciotorres_meta_desc',
        '_yoast_wpseo_opengraph-title' => '_luciotorres_og_title',
        '_yoast_wpseo_opengraph-description' => '_luciotorres_og_desc',
        '_yoast_wpseo_opengraph-image' => '_luciotorres_og_image',
        '_yoast_wpseo_canonical' => '_luciotorres_canonical',
    ];
    ...
    /**
     * Map Yoast _yoast_wpseo_* meta to _luciotorres_* meta.
     *
     ...
     * @param  array<string, string>  $existing  Existing _luciotorres_* values (to skip)
     * @return array<string, string>  Mapped luciotorres meta key-value pairs
     */
    public static function mapYoastMeta(
        ...
        foreach (self::META_MAP as $yoastKey => $luciotorresKey) {
            ...
            if (isset($existing[$luciotorresKey]) && $existing[$luciotorresKey] !== '') {
                $mapped[$luciotorresKey] = $existing[$luciotorresKey];
                continue;
            }

            $mapped[$luciotorresKey] = $value;
        }
        ...
        if (isset($yoastMeta['_yoast_wpseo_robots'])) {
            ...
            if (! isset($existing['_luciotorres_noindex']) || $existing['_luciotorres_noindex'] === '') {
                $mapped['_luciotorres_noindex'] = $noindex;
            }
        }
    ...
    /**
     * WP-CLI command handler for `wp luciotorres migrate-seo`.
     *
     * Iterates over all published posts and pages, reads Yoast meta,
     * and writes _luciotorres_* keys.
     *
     * ## OPTIONS
     *
     * [--dry-run]
    ...
    public static function handle(array $args, array $assocArgs): void
    {
        ...
        foreach ($chunks as $chunk) {
            ...
            foreach ($chunk as $postId) {
                ...
                // Read existing _luciotorres_* values to avoid overwriting
                $existing = [];
                foreach (self::META_MAP as $luciotorresKey) {
                    $val = get_post_meta($postId, $luciotorresKey, true);
                    if ($val !== '' && $val !== false) {
                        $existing[$luciotorresKey] = $val;
                    }
                }
                // Also check noindex
                $noindexVal = get_post_meta($postId, '_luciotorres_noindex', true);
                if ($noindexVal !== '' && $noindexVal !== false) {
                    $existing['_luciotorres_noindex'] = $noindexVal;
                }
